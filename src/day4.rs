...
fn probe_direction(field: &Vec<Vec<char>>, line : isize, col : isize, dl: isize, dc: isize, expect: char) -> i32 {
    let new_l = line + dl;
    let new_c = col + dc;

    if new_l < 0 { return 0; }
    if new_c < 0 { return 0; }
    if new_l as usize  >= field.len() { return 0; }
    if new_c as usize >= field[0].len() { return 0; }
        ...
        for li in 1 .. self.field.len()-1 {
            for ci in 1 .. self.field[0].len()-1 {
                if self.field[li][ci] == 'A' {
                    let diag1 = (self.field[li-1][ci-1] == 'M' && self.field[li+1][ci+1] == 'S') ||
                                      (self.field[li-1][ci-1] == 'S' && self.field[li+1][ci+1] == 'M');
                    let diag2 = (self.field[li-1][ci+1] == 'M' && self.field[li+1][ci-1] == 'S') ||
                                      (self.field[li-1][ci+1] == 'S' && self.field[li+1][ci-1] == 'M');
                    if diag1 && diag2 { sum += 1; }
                }
            }
        }
